...
client_control_qoes = loadAllSessionQoE(client_control_files, QoE="cascading")
server_control_qoes = loadAllSessionQoE(server_control_files, QoE="cascading")
dash_qoes = loadAllSessionQoE(dash_files, QoE="cascading")

percentile = 0.9
fig, ax = plt.subplots()
dash_percentile_qoe = draw_cdf(ax, dash_qoes, plt_styles[0], "DASH Client with \nRTT based Server Selection", percentile)
client_percentile_qoe = draw_cdf(ax, client_control_qoes, plt_styles[1], "Client-side Control", percentile)
server_percentile_qoe = draw_cdf(ax, server_control_qoes, plt_styles[2], "Server-side Control", percentile)

ax.set_xlabel(r'Session QoE', fontsize=15)
ax.set_ylabel(r'The percentage of users', fontsize=15)
ax.set_title('The CDF of user session QoE', fontsize=15)
...
save_fig(fig, imgFolder+"control_exp_cdf", ".all")
