## Draw the chunk QoE over time for multiple clients
# Compare clients' QoE under dynamic server capacity.
# Show client's QoE the byzantine faults server have impacted.
# showQoEwithFaults.py
from draw_chunk_QoEs import *

dataFolder = '/Users/Chen/Box Sync/Proposal/Chen\'s Proposal/exps/byzantineFaults/by_type2_0823/'
...
for client_file in dash_client_files:
	...
	if client_filter in client:
		...
		plt.step(ts, qoes, plt_styles[clientID], label=client_name if client_name not in legend_list else None, linestyle=line_styles[clientID], linewidth=2.0, markersize=8)
...
pdf = PdfPages(dataFolder + 'imgs/byzantine_type2_clients_QoE.pdf')
pdf.savefig(fig)
