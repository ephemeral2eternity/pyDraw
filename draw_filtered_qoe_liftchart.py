...
filter_obj = dict(filter_key="Server", filter_value="cache-01")
...
ax.set_title('The Lift Chart of user session QoE filtered by ' + filter_obj["filter_value"], fontsize=20)
plt.legend(bbox_to_anchor=(0.8, 0.4))
plt.show()

pdf = PdfPages('./imgs/qoe_liftchart_filtered_by_' + filter_obj["filter_key"] + '_' + filter_obj["filter_value"] + '.pdf')
pdf.savefig(fig)
